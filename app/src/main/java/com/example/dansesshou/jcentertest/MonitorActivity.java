...
public class MonitorActivity extends BaseMonitorActivity {
        ...
        rxPermissions.request(Manifest.permission.CAMERA)
                .subscribe(granted -> {
                    if (granted) { // 在android 6.0之前会默认返回true
                        // 已经获取权限
                        Log.e("MonitorActivity", "已授予CAMERA权限");
                    } else {
                        // 未获取权限
                        ToastUtils.ShowError(MonitorActivity.this, "您没有授权CAMERA权限，请在设置中打开授权", Toast.LENGTH_SHORT, true);
                    }
                });
    }
            ...
            rxPermissions.request(Manifest.permission.RECORD_AUDIO)
                    .subscribe(granted -> {
                        if (granted) { // 在android 6.0之前会默认返回true
                            ...
                            // 未获取权限
                            ToastUtils.ShowError(MonitorActivity.this, "您没有授权麦克风权限，请在设置中打开授权", Toast.LENGTH_SHORT, true);
                        }
                    });
        }
    }
        ...
        rxPermissions.request(Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                .subscribe(granted -> {
                    if (granted) { // 在android 6.0之前会默认返回true
                        ...
                        Log.e("dxsTest", "d:" + d);
                        captureScreen(-1);
                        Log.e("MonitorActivity", "已授予STORAGE权限");
                    } else {
                        // 未获取权限
                        ToastUtils.ShowError(MonitorActivity.this, "您没有授权STORAGE权限，请在设置中打开授权", Toast.LENGTH_SHORT, true);
                    }
                });
    }
    ...
    public void startMoniterRecoding() {
        ...
        try {
            ...
            Toast.makeText(MonitorActivity.this, " 没有内存卡", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    ...
    public void stopMoniterReocding() {
        if (P2PHandler.getInstance().stopRecoding() == 0) {
            //录像不正常
            Toast.makeText(MonitorActivity.this, " 录像不正常", Toast.LENGTH_SHORT).show();
        } else {
            //正常停止
            Toast.makeText(MonitorActivity.this, " 停止录像", Toast.LENGTH_SHORT).show();
        }
    }
